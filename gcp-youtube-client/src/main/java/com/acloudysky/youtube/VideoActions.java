...
public class VideoActions {
     ...
     */
    private static YouTube youtubeServiceClient;
    
    private static AuthenticateGoogleServiceClient serviceAuthentication;
    
    private static YoutubeDefaultSettings defaultSettings; 
    ...
    public static void initVideoActions(YouTube authorizedService, AuthenticateGoogleServiceClient authentication, 
    		YoutubeDefaultSettings clientDefaultSettings ) {
		
 		// Initialize authorized youtube service.
     	youtubeServiceClient = authorizedService;
    ...
    ***********/
    ...
    /**
     * Compares new scope against the current one.
     * If the new scope is different from the current one, a new authenticated client
     * is created along with a new StoredCredential file.   
     * If the StoredCrential was created with a different scope, a 403 error is issued 
     * For more information, 
     * see <a href="http://www.w3.org/Protocols/rfc2616/rfc2616-sec10.html">access forbidden</a>
     * @param scope The new scope. 
     */
    private static void checkScopes(String scope) {
    	
    	String currentScope = defaultSettings.getCurrentScope();
    	
    	if (!scope.equals(currentScope)) {
    		String selectedScope = 
    				serviceAuthentication.getScope(IGoogleServiceClientAuthentication.youtubeScopes, scope);
    	
    		// The new scope is different from the stored one; ask for owner's grant request.
    		try {	
    				// Get the authenticated client by asking for Youtube owner's grant request.
    				// Passing false allows for the deletion of the StoredCredential file which force 
    				// a new Youtube owner's grant request.
        			youtubeServiceClient = 
        					serviceAuthentication.getAuthenticatedYouTubeClient(selectedScope, false);
        			
        			// Update the current scope in the default settings. 
        			defaultSettings.updateDefaultSettings("currentscope", scope);
	    	   
	    		}catch(Exception e){
	    			int httpError = ((GoogleJsonResponseException) e).getDetails().getCode();
	    			if (httpError == 403) {
	    				System.err.println("403 error issued.");
        		}
	    		e.printStackTrace();
    ...
    public static void uploadVideo() {
    	...
    	try {
        		// Compare new scope against the current one. 
    			checkScopes("youtube_upload");
	            ...
	            InputStream inStream = VideoActions.class.getResourceAsStream("/" + SAMPLE_VIDEO_FILENAME);
	            InputStreamContent mediaContent = new InputStreamContent(VIDEO_FILE_FORMAT, inStream);
	            ...
	            YouTube.Videos.Insert videoInsert = youtubeServiceClient.videos()
	            ...
	            displayVideoInformation(uploadedVideo);
	        		...
	        		if (httpError == 403) {
	        			System.err.println("Error 403 issued.");
	        		}
	        		else  
...
//            System.exit(1);
    ...
    public static void updateVideo() {
        ...
        try {
	            ...
	            YouTube.Videos.List listVideosRequest = youtubeServiceClient.videos().list("snippet").setId(videoId);
	            ...
	            YouTube.Videos.Update updateVideosRequest = youtubeServiceClient.videos().update("snippet", video);
